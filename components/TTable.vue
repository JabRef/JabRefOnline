...
<template>
  <table :class="tableClass">
    <thead>
      <tr>
        <th v-for="header in headers" :key="header" :class="headerClass">
          {{ header }}
        </th>
      </tr>
    </thead>
    <tbody :class="tbodyClass">
      <tr v-for="(row, index) in data" :key="index">
        <td v-for="cell in row" :key="cell" :class="tdClass">
          {{ cell }}
        </td>
      </tr>
    </tbody>
...
let headerClass = 'px-3 py-2 font-semibold text-left bg-gray-100 border-b'
